...
class ChainsawAudioModel:
    ...
    def __init__(self, **overrides):
        ...
        self.sr = None  # Будет установлен в generate()
    ...
    def _rpm_profile(self, duration, sr, throttle_steps=None):
        """Динамика оборотов с учётом дросселя, нагрузки и ограничителя"""
        t = np.arange(0, duration, 1.0 / sr)
        if throttle_steps is None:
            throttle_steps = [(0.0, 1.0), (0.2, 1.0), (0.25, 0.3), (0.6, 1.0)]

        rpm_target = np.zeros_like(t)
        current_rpm = self.params["idle_rpm"]

        for start, thr in throttle_steps:
            mask = t >= start
            if not np.any(mask):
                continue
            idx = np.argmax(mask)
            target = (
                ...
                + (self.params["governor_max_rpm"] - self.params["idle_rpm"]) * thr
            )
            tau = self.params["throttle_tau_s"] * (
                1.0 + 0.5 * self.params["cutting_load"]
            )
            dt = 1.0 / sr
            for i in range(idx, len(t)):
                droop = (
                    self.params["cutting_load"]
                    * 1200.0
                    * np.sin(2 * np.pi * 0.5 * t[i])
                )
                rpm_target[i] = current_rpm + dt / tau * (target + droop - current_rpm)
                current_rpm = rpm_target[i]

        return t, np.clip(
            rpm_target, self.params["idle_rpm"] - 200, self.params["governor_max_rpm"]
        )

    def _combustion_signal(self, t, rpm, sr):
        """Импульсы сгорания + гармоники с физически обоснованным затуханием"""
        p = self.params
        ...
        # Гармоники сгорания — ИСПРАВЛЕНА ПРОВЕРКА НАЙКВИСТА
        n_harmonics = 40
        spectral_rolloff = (
            1.2
        ...
        for n in range(1, n_harmonics + 1):
            # ✅ Используем max(rpm) для скалярной проверки частоты
            f_n_max = (np.max(rpm) / 60.0) * n
            if f_n_max > sr / 2:
                break
            # ✅ Но для генерации используем массив мгновенных частот
            f_n = (rpm / 60.0) * n
    ...
    def _mechanical_noise(self, t, rpm, sr):
        ...
        # ✅ Используем среднее RPM для расчёта частоты резонанса (скаляр)
        rpm_mean = np.mean(rpm)
        f_mech = 120.0 + 80.0 * (rpm_mean / 10000.0)
        Q = 2.5 + 1.5 * (p["conrod_length_mm"] / 95.0)

        # Защита от некорректных границ фильтра
    ...
    def _chain_noise(self, t, rpm, sr):
        ...
        Q_chain = 4.0 - 2.0 * p["chain_wear_factor"]  # Теперь используется!
    ...
    def _apply_acoustic_filters(self, signal, sr):
        ...
        p = self.params
        # Глушитель: НЧ-фильтр + резонанс выхлопа
        ...
        # Резонанс корпуса (полосовой фильтр)
    ...
    def generate(
        self, duration=8.0, sr=44100, throttle_profile=None, filename="chainsaw.wav"
    ):
        self.sr = sr  # Сохраняем для доступа из других методов при необходимости
        t, rpm = self._rpm_profile(duration, sr, throttle_profile)
        ...
        print(
            f"✅ Звук сохранён в {filename} | Длительность: {duration}с | Семплов: {len(mix)}"
        )
        return t, rpm, mix
...
if __name__ == "__main__":
    cfg = {
        ...
        "air_fuel_ratio": 12.5,
        "cutting_load": 0.85,
        "chain_tension_N": 60.0,
        "sprocket_teeth": 7,
        "guide_bar_len_cm": 50,
        "governor_max_rpm": 14200,
        "muffler_volume_cm3": 600,
        "throttle_tau_s": 0.08,
        "mic_distance_m": 0.8,
        "directional_gain_db": 5.0,
    }

    profile = [(0.0, 0.0), (1.5, 1.0), (2.0, 1.0), (5.5, 0.2), (6.5, 1.0)]

    model = ChainsawAudioModel(**cfg)
    t, rpm, audio = model.generate(
        duration=8.0, sr=44100, throttle_profile=profile, filename="chainsaw_demo.wav"
    )
